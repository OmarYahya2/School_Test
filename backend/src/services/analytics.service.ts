...
export class AnalyticsService {
  ...
  static async buildSummary() {
    ...
    const [
      ...
      prisma.attendanceRecord.findMany({ select: { records: true } }),
      prisma.grade.findMany({ select: { grade: true, maxGrade: true } }),
    ...
    for (const record of allAttendance) {
      const recs = record.records as { present: boolean }[];
    ...
    const gradePassRatePct =
      allGrades.length > 0
        ? Math.round((passCount / allGrades.length) * 100)
        : null;
    const gradeAveragePct =
      allGrades.length > 0
    ...
    const teacherClassIds = new Set(
      classesRaw.filter((c) => c.teacherId).map((c) => c.id)
    );
    const studentsInTeacherClasses = allStudents.filter((s) =>
      teacherClassIds.has(s.classId)
    ).length;
    ...
    const classBreakdown = classesRaw
      .map((cls) => {
        ...
        return {
          ...
          averageAge:
            cls_students.length > 0
              ? Math.round(
                  (cls_students.reduce((sum, s) => sum + s.age, 0) /
                    cls_students.length) *
                    10
                ) / 10
              : 0,
